    ...
    return ( <
        ...
        div className = "app" > {
            this.sortByName(this.tagDuplicate(shadows), "asc").map((anObjectMapped, index) => {
                return ( <
                    Bloc shadow = { anObjectMapped.shadow }
        ...
        <
        ColorPicker status = { this.state.picker }
...
const Title = ({ isDarkBackground }) => {
    return ( <
        ...
        <
        div className = "logo"
        title = "Handpicked Box Shadows" > Box Shadows < /div> { /* <p className="subtitle" style={isDarkBackground ? {color: Color('#777').negate()}: {}}>Handpicked Box Shadows</p> */ } <
        a className = "addnew"
        target = "_blank"
...
class GoogleBar extends Component {
    ...
    render() {
        return ( <
            ...
            box - shadow: 0 2 px 2 px 0 rgba(0, 0, 0, 0.16), 0 0 0 1 px rgba(0, 0, 0, 0.08); <
            /span> <
